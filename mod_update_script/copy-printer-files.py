...
website_printers_dir = os.path.join(script_dir, '../website/_printers')
# Make sure the printers collection directory exists
os.makedirs(website_printers_dir, exist_ok=True)
...
def download_and_create_metadata(url, title, description, project_name, file_name):
    ...
    # Create a per-printer directory under _printers using the project key
    printer_dir = os.path.join(website_printers_dir, project_name)
    os.makedirs(printer_dir, exist_ok=True)

    file_path = os.path.join(printer_dir, filename)
    ...
    if response.status_code == 200:
        ...
        # layout/printer_id/permalink align with site's printer layout
        permalink = f"/printers/{project_name}/"
        metadata = f"""---
layout: "printer"
title: "{title}"
description: "{description}"
printer_id: "{project_name}"
permalink: "{permalink}"
repository: "{url}"
---
"""
        ...
        with open(file_path, "w") as f:
            f.write(text)
        print(f"Downloaded {filename} to {file_path} with frontmatter")
...
def parse_config(config_file):
    ...
    for project_name, project_info in config.items():
        ...
        # Require markdown output compatible with Jekyll collection
        if not file_name or not file_name.endswith('.md'):
            print(f"Skipping project {project_name} due to invalid or missing file name")
            continue
